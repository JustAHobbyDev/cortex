...
- specs/ - versioned, immutable once activated (new version required for changes).
...
- DEPRECATE (for inv/ and specs/ references; never delete)
- PROPOSE (non-mutating suggestion packaged for review)
- ACTIVATE (promote specs/*_vN to active)
...
- PROPOSE on project/* and specs/* (no direct activation by default)
...
### Specs
- specs/*_v0 becomes active only via ACTIVATE authority.
